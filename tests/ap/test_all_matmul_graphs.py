...
def test_all_graphs(log_dir, test_all_case = False):
    ...
    gpu_id = '2'
    for file_name in os.listdir(dir_name + test_dir):
        # For phase1-2 cases:
        ...
        if file_name.endswith(test_case_type):
            for i in range(3):
                file_id = str(i+1)
                cmd = ["bash", shell_script + f"/tests/ap/auto{file_id}.sh", file_seq_num, log_dir, gpu_id]
                try:  #!! pack them
                    time_begin = time.time()
                    rcode = run(cmd, timeout=300)
                    time_end = time.time()
