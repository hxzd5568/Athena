...
import matmul_variadic_split_tpl
...
@abstract_drr.register_drr_pass("matmul_split_fusion", nice=0)
class MatmulBinaryFusion(abstract_drr.DrrPass):
  def source_pattern(self, o, t):
    ...
    o.trivial_op2 = o.ap_trivial_fusion_op()
    o.trivial_op2(
        [t.mm_out],
        [t.output0]
    )
  ...
  def result_pattern(self, o, t):
    ...
    o.fustion_op([t.input0, t.input1], [t.output0, t.output1, t.output2])
  ...
  def _make_kernel_arg_translator(self):
    return matmul_variadic_split_tpl.make_kernel_arg_translator()
  ...
  def _simplify_index_program(self, mut_program):
    ...
    drr_pass = topo_drr_pass.ConvertDownSpiderSliceStoreDataOpToYieldOpPass()
    pass_manager.add_pass(ir_tools.create_access_topo_drr_one_step_pass(drr_pass))
    pass_manager.add_pass(ir_tools.create_dce_pass())
    pass_manager.run(mut_program)
    return mut_program
  ...
  def _make_index_func_unique_id2index_program(
    ...
    def MatchAndCopyOutputIndex(dst_output_name):
        pass_manager = ir_tools.create_pass_manager()
        removed_programs = MutableList()
        drr_pass = matmul_epilogue_pass.RemoveSliceOutputIndexPass(
            src_data_op_name=anchor_data_op_name,
  ...
  def _get_program_translator(self, ctx, o, t):
    # m_list = MutableList()
    # m_list.append(o.trivial_op)
    # m_list.append(o.trivial_op1)
    # mut_program = ir_tools.append_fused_ops_to_program(
    #   packed_args_val = m_list, tensor_match_ctx=t
    # )
    ...
    index_program_translator_map = index_program_translator_util.IndexProgramTranslatorMap(
      ...
      anchor_iter_var_names=matmul_variadic_split_tpl.get_anchor_iter_var_names(mm_out_symbolic_shape),
      anchor_iter_dim_splits=matmul_variadic_split_tpl.get_anchor_iter_dim_splits(mm_out_symbolic_shape),
    )
